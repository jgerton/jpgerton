...
must_haves:
  ...
  artifacts:
    - path: "app/globals.css"
      provides: "CSS keyframe animations (fade-up, fade-in), prefers-reduced-motion global disable, focus ring transition"
...
2. **Animation utility classes** in `@layer utilities`:
   - `.animate-fade-up` using the fade-up keyframe with var(--duration-base) and var(--ease-smooth), fill mode `both`
   - `.animate-fade-in` using the fade-in keyframe with var(--duration-base) and var(--ease-smooth), fill mode `both`
...
- Duration ceiling: no animation longer than ~200-250ms unless hero/onboarding
...
- Nothing loops unless communicating "waiting" or "live" state
  ...
  <verify>Run `bun run build` and confirm no CSS errors. Grep globals.css for "@keyframes fade-up", "@keyframes fade-in", "prefers-reduced-motion: reduce", and "focus-visible" transition to confirm all four sections are present.</verify>
  <done>globals.css contains fade-up and fade-in keyframes, animation utility classes, focus ring transition, and a prefers-reduced-motion media query that disables all animations globally.</done>
</task>
...
3. globals.css contains prefers-reduced-motion: reduce media query with global animation/transition disable
4. globals.css contains focus-visible transition for ring animation
5. button.tsx base class includes active:scale-[0.97] and transform in transition
6. Existing hover states (card lg elevation, button gradient, nav underline) are unchanged
</verification>

<success_criteria>
- CSS keyframe animations defined and available as utility classes
