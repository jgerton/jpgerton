...
**Barrel export (components/portfolio/index.ts):**
```typescript
...
export type { ProjectCardEnhancedProps } from "./project-card-enhanced"; // if interface is exported
...
Note: Only include `type` exports for interfaces that are actually exported from their source files. Check each source file before writing the barrel export. If a component doesn't export its props interface, just export the component itself.

**Consistency verification (do this as part of the task, fix any issues found):**
...
If any inconsistency is found, fix it in the source file. Document what was found and fixed in the summary.
  ...
  <verify>
Run `bun run build` and confirm zero TypeScript errors. Verify barrel export works:
...
4. All composition components follow consistent card elevation convention (flat/sm/lg)
5. All composition components use spacing tokens (no arbitrary px values)
6. All composition components use semantic color variables (no hardcoded hex)
7. All composition components use font-serif for headings/values
</verification>
